...
public class RaftNodeBuilderImpl implements RaftNodeBuilder {
    ...
    public RaftNode build() {
        ...
        TLATracer spec = null;
        try {
            String nodeName = localEndpoint.getId().toString();
            spec = TLATracer.getTracer(nodeName + ".ndjson", ClockFactory.getClock(ClockFactory.FILE, "raft.clock"));
        } catch (IOException | ClockException e) {
            e.printStackTrace();
        }

        if (restoredState != null) {
            return new RaftNodeImpl(groupId, restoredState, config, executor, stateMachine, transport, modelFactory,
                    store, listener, random, clock, spec);
        } else {
            ...
            return new RaftNodeImpl(groupId, localEndpoint, groupMembers, config, executor, stateMachine, transport,
                    modelFactory, store, listener, random, clock, spec);
        }
    }
