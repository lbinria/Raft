...
public final class ReplicateTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            /*
             * SpecHelper.getLogVariable(raftNode.getLocalEndpoint().getId().toString()).
             * apply("AppendElement", tlaEntry);
             * SpecHelper.get(raftNode.getLocalEndpoint().getId().toString()).commitChanges(
             * "ClientRequest", new Object[]{raftNode.getLocalEndpoint().getId().toString(),
             * entry.getOperation().toString()}); System.out.println("CLIENT REQUEST");
             */
