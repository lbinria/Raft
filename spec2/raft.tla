...
Index == Nat   \* override for model checking
...
Init ==
    ...
    /\ term = [s \in Server |-> 0]
    /\ ballots = [s \in Server |-> {}]
    /\ ghostEntries = [s \in Server |-> [n \in Nat |-> {}]]
...
(*
    /\ \/ lastEntryTerm(s) < lastEntryTerm(cdt)
       \/ lastEntryTerm(s) = lastEntryTerm(cdt) /\ Len(entries[s]) <= Len(entries[cdt])
*)
